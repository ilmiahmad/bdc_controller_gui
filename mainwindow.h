#ifndef MAINWINDOW_H
#define MAINWINDOW_H

#include <QMainWindow>
#include "qcustomplot.h"

#include <QSerialPort>
#include <QTimer>
#include <QDebug>

const /*_rom*/ quint8 mxCRC8_table[256] = {
     };

enum packetCode {
    OPERATION,    // 0
    READPARAM,      // 1
    SAVECONFIG    // 2
};

enum controlMode    {
    PWM,
    SPEED,
    POSITION
};

typedef struct Motor_Parameter
{
    quint8 devId;
    float gearBox;
    quint16 encoderCpr;
    qint8 encoderDir;
    quint8 freqMode;

    // Control Parameter
    quint8 controlMode;
    float posKp;
    float speedKp;
    float ki;
    qint16 maxPwm;
    float maxSpeed;

    //recv parameter
    float m_current;
    float m_speed;
    float m_position;
}Motor;


typedef union FloatBytes {
    float floatValue;
    quint8 byteArray[4];
}float2bytes;

typedef union int16_tBytes {
    int16_t int16_tValue;
    uint8_t byteArray[2];
}int16_t2bytes;

QT_BEGIN_NAMESPACE
namespace Ui { class MainWindow; }
QT_END_NAMESPACE

class MainWindow : public QMainWindow
{
    Q_OBJECT

public:
    MainWindow(QWidget *parent = nullptr);
    void setupPlot();
    void getFromGUIParam();
    ~MainWindow();

signals:
    void dataPlotChanged();
    void dataParamChanged();
    void dataReady(QByteArray data_in);
    void calibrationDone();

private slots:

    void recvHandler();
    void targetcallback();
    bool saveConfiguration();
    void updateGUIParamData();
    void updatePlotData();
    void requestParam();
    void sendParam();
    void processData(QByteArray data_in);
    void on_applytarget_button_clicked();
    void on_connect_button_clicked();
    void on_calibration_button_clicked();
    void on_save_button_clicked();
    void on_apply_button_clicked();
    void calibrationProcessData();

private:

    void writeData(const QByteArray &data);
    void sendTarget(quint8 mode, float data);
    bool retrieveData(Motor data);
    void addRealtimeSample(double v);
    void readData();
    void showStatusMessage(const QString &message);
    void showWriteError(const QString &message);

    quint8 calculateCRC8(QByteArray &data);

    Ui::MainWindow *ui;
    Motor *motor = nullptr;
    QSerialPort *serial = nullptr;
    QTimer *stimer = nullptr;
    QTimer *calibTimer = nullptr;

    const double dt = 0.05;
    double t = 0;
    int header_size = 5;

    bool calibrationState = false;
};



#endif // MAINWINDOW_H
